...
class DemoUserApiMiddleware
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($user && $user->email === 'demo@example.com') {
            ...
            if (in_array($request->method(), ['POST', 'PUT', 'PATCH', 'DELETE'])) {
                ...
                if ($validationResult['success']) {
                    return response()->json([
                        'success' => true,
                        'message' => 'Validation passed, but demo users cannot perform this action. Only logout is allowed.',
            ...
            // // Block GET requests to modification routes
            // return response()->json([
            //     'success' => false,
            //     'message' => 'Demo users can only logout. All other actions are restricted.'
            // ], 403);
        }
